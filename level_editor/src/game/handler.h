...
class handler
{
    ...
    player user;
    bool intangible;
    utilities::countdown jump_buffered_countdown;
...
public:
    handler(math::vec<int, 2> window_dimensions) : texts{graphics::game_textures::get_instance()},
    ...
    bool load_file(const std::vector<editor::state> &world, math::dvec2 spawn, math::dvec2 end)
    {
        blue_objs.clear();
        red_objs.clear();
        neutral_objs.clear();
